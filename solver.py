...
class Board():
    ...
    def evaluateSpace(self, space):
        options = [i+1 for i in range(self.n2)]
        for taken in self.valsInRows[space[0]]:
            if(taken in options):
                options.remove(taken)
            if(len(options) == 0): break
        for taken in self.valsInCols[space[1]]:
            if(taken in options):
                options.remove(taken)
            if(len(options) == 0): break
        for taken in self.valsInBoxes[self.spaceToBox(space[0],space[1])]:
            if(taken in options):
                options.remove(taken)
            if(len(options) == 0): break
        return options
    ...
    def getMostConstrainedUnsolvedSpace(self):
        if len(self.unsolvedSpaces) == 0: return None
        picky = next(iter(self.unsolvedSpaces))
        options = self.evaluateSpace(picky)
        for space in self.unsolvedSpaces:
            tempOptions = self.evaluateSpace(space)
            if len(tempOptions) < len(options):
                picky = space
                options = tempOptions
        return picky
...
class Solver:
    ...
    def solveBoard(self, board):
        if(len(board.unsolvedSpaces) == 0):
            ...
            for space in board.unsolvedSpaces:
                space = board.getMostConstrainedUnsolvedSpace()
                options = board.evaluateSpace(space)
                if(len(options) == 0):
                    return False
                else:
                    for opt in range(0, len(options)):
                        if(board.isValidMove(space,options[opt])):
                            board.makeMove(space, options[opt])
                            if(self.solveBoard(board)):
                                return True
                            else:                                
                                board.undoMove(space, options[opt])
                    return False
            return False
